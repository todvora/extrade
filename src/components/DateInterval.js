...
export default class DateInterval extends React.Component {
 ...
 onDateTillChange (newDate) {
  ...
  getIntervalLength () {
    moment.locale('cs')
    var start = moment([this.props.interval.from.year, this.props.interval.from.month - 1, 0])
    var end = moment([this.props.interval.till.year, this.props.interval.till.month - 1, 1]).endOf('month')
